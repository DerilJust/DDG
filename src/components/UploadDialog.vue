<template>
  <el-dialog v-model="dialogVisible" title="上传并生成图纸" width="90%" :close-on-click-modal="false" @close="handleCancel">
    <div class="upload-dialog-body">
            ...
            <el-form label-width="80px" label-position="left">
              <el-form-item label="宽度">
                <div class="slider-row">
                  <el-input-number v-model="gridWidth" :min="1" :max="1000" :step="1" :controls="true" size="small"
              ...
              <el-form-item label="高度">
                <div class="slider-row">
                  <el-input-number v-model="gridHeight" :min="1" :max="1000" :step="1" :controls="true" size="small"
                    class="custom-input-number" />
      ...
      <div class="dialog-right-panel">
        <el-upload class="dialog-upload" :auto-upload="false" :on-change="handleFileChange" :show-file-list="false"
          accept="image/*">
    ...
    <template #footer>
      ...
      <el-button type="primary" @click="handleConfirm" :disabled="!selectedFile || !cropRect.visible"
...
const {
  gridWidth, gridHeight, colorCount,
  selectedBrand, showNumbers, lockAspectRatio
...
const cropRect = ref<{
  ...
  visible: false, dragging: false, resizing: false, resizeDirection: ''
});
...
const initCropRect = (): void => {
  ...
  if (lockAspectRatio.value) {
    ...
    cropRect.value = {
      ...
      visible: true, dragging: false, resizing: false, resizeDirection: ''
    };
  } else {
...
const handleResize = (mouseX: number, mouseY: number) => {
  ...
  if (direction.includes('l')) { const deltaX = mouseX - start.x; newWidth = original.width - deltaX; newX = original.x + deltaX; }
  if (direction.includes('r')) { const deltaX = mouseX - start.x; newWidth = original.width + deltaX; }
  if (direction.includes('t')) { const deltaY = mouseY - start.y; newHeight = original.height - deltaY; newY = original.y + deltaY; }
  if (direction.includes('b')) { const deltaY = mouseY - start.y; newHeight = original.height + deltaY; }
...
const cropCanvasImage = (img: CropperImageData, crop: any): Promise<CroppedResult> => {
  return new Promise((resolve, reject) => {
    ...
    if (!tempCtx) { reject(new Error('无法获取Canvas上下文')); return; }
    ...
    tempCanvas.toBlob((blob) => {
      ...
      croppedImage.onload = () => { resolve({ image: croppedImage, dataUrl, file: croppedFile }); };
      croppedImage.onerror = () => { reject(new Error('裁剪图片加载失败')); };
...
.crop-handle.top { top: -6px; left: 50%; transform: translateX(-50%); cursor: n-resize; }
.crop-handle.bottom { bottom: -6px; left: 50%; transform: translateX(-50%); cursor: s-resize; }
.crop-handle.left { top: 50%; left: -6px; transform: translateY(-50%); cursor: w-resize; }
.crop-handle.right { top: 50%; right: -6px; transform: translateY(-50%); cursor: e-resize; }
...
.crop-handle.top:hover { transform: translateX(-50%) scale(1.2); }
