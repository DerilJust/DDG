...
export const getPendingCells = (
  ...
  const x1 = Math.max(
    0,
    Math.min(gridWidth - 1, Math.min(pendingSelection.x1!, pendingSelection.x2!))
  )
  const y1 = Math.max(
    0,
    Math.min(gridHeight - 1, Math.min(pendingSelection.y1!, pendingSelection.y2!))
  )
  const x2 = Math.max(
    0,
    Math.min(gridWidth - 1, Math.max(pendingSelection.x1!, pendingSelection.x2!))
  )
  const y2 = Math.max(
    0,
    Math.min(gridHeight - 1, Math.max(pendingSelection.y1!, pendingSelection.y2!))
  )
